...
def ext_pillar(minion_id, pillar, *args, **kwargs):
    ...
    device_results = salt.utils.http.query(device_url,
                                           ...
                                           header_dict=headers,
                                           decode=True)
    vm_url = '{api_url}/{app}/{endpoint}'.format(api_url=api_url,
                                                 app='virtualization',
                                                 endpoint='virtual-machines')
    vm_results = salt.utils.http.query(vm_url,
                                       params={'name': minion_id},
                                       header_dict=headers,
                                       decode=True)
    ...
    if len(devices) == 1:
        if len(vms) == 0:
            ...
            log.error('One device found for "%s", but also virtual-machine matches', minion_id)
            return ret
    elif len(devices) > 1:
        ...
        if len(vms) == 1:
            ...
            log.error('More than one virtual-machine found for "%s"', minion_id)
            return ret
        else:
            log.error('Unable to pull NetBox data for "%s"', minion_id)
            return ret
    interfaces = {}
    if device_id and device_interfaces:
